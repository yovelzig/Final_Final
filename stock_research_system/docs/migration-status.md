...
| A2 | Production image and worker-health correction | Complete (local verification only — not yet deployed to EC2) |
...
**Phase A2 is complete (local verification only).** The next phase to start is **B — Curriculum and first usable learner flow** (not started).
...
- **Phase B entry criteria:** Phase A2's local verification is complete; before Phase B (curriculum work) begins, a human operator should deploy Phase A2 to EC2 following `production-deployment-runbook.md` §7 and confirm worker health in production, since Phase B's curriculum-seeding work will exercise `finquest-worker-default`/the seed scripts against the real environment.
