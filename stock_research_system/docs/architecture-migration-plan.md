...
| Tests | Layered backend unit/integration and frontend unit/component/integration/accessibility/E2E suites exist. | ✅ | add tests for Ollama-real-endpoint path, misconception detection once built | — | ruff/mypy/black still absent; CI (`.github/workflows/ci.yml`, added Phase A1) runs backend unit tests, frontend typecheck/lint/test, and both Compose-config validations, but not integration tests, Playwright E2E, or `npm run api:check` | 🟡 (CI exists but integration/E2E/OpenAPI-freshness gaps remain) | 1 (CI added, done); follow-up phase to add integration/E2E/api:check gating |
...
| CI exists, but integration, E2E, and OpenAPI freshness are not gated | Confirmed | Medium (integration-test regressions, E2E regressions, or OpenAPI/frontend-type drift can land undetected) | `.github/workflows/ci.yml` exists (backend job: pip install + `pytest tests/unit -q`; frontend job: `npm ci` + `typecheck` + `lint` + `test`; compose-validation job: both Compose files via `config --quiet` with placeholder env); `ruff`/`mypy`/`black` still not installed or configured; CI does not run `pytest tests/integration`, `npm run test:e2e`, or `npm run api:check` | The original Stage 1 CI proposal is done — this closes it. A later phase should add integration-test, Playwright E2E, and `api:check` gating to CI. | 1 (done); follow-up phase for integration/E2E/api:check gating |
