...
| Delete `docker-compose.yml.backup` | Confirm with the user the one-line `HOSTNAME` divergence was an intentional, already-adopted change (see `deprecation-removal-plan.md` §4) |
| Enable `LANGGRAPH_ENABLED=true` in any shared environment | Fix the 2 failing `test_openapi_snapshot.py` assertions (update the allowlist deliberately), verify the frontend deployed alongside it also expects `/api/v1/coach` to exist (see §5 risk above) |
