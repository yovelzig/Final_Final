...
must use one of the exact approved sentences, refusal/fallback guardrail
decisions must carry a non-blank override). Phase G2E2A adds Hebrew
counterparts of these exact strings alongside the English ones - the
"exact approved sentence" invariant applies identically to both.
"""
...
# Phase G2E2A: exact, approved Hebrew counterparts of the three safety
# strings above. Selected by the shared `LanguageServicePort.localize()`
# (never freshly generated by a model) whenever a learner's detected
# language is Hebrew - the same "exact approved sentence" invariant the
# English strings already carry applies here too (see
# `TutorAnswer._validate_answer` and `TutorGuardrailDecision._validate_decision`
# below).
EXACT_INSUFFICIENT_EVIDENCE_FALLBACK_HE = (
    "אין לי מספיק חומר מאושר של FinQuest כדי לענות על כך באופן אמין."
)
EXACT_ADVICE_REFUSAL_HE = (
    "אני יכול להסביר את המושגים, הסיכונים והדוגמאות החינוכיות, אך אינני יכול לומר לך "
    "מה לקנות, למכור או במה להשקיע באופן אישי."
)
EXACT_SCENARIO_FUTURE_INFORMATION_REFUSAL_HE = (
    "אני יכול לעזור לך להעריך את הסיכונים, אופק הזמן, הפיזור והמידע שהיה זמין בנקודת "
    "ההחלטה, אך אינני יכול לחשוף את התוצאה העתידית או לציין מהי האפשרות הנכונה."
)

#: Every exact fallback string a `FALLBACK` `TutorAnswer` may legally
#: carry, across every supported language. `_validate_answer` below
#: checks membership in this set rather than equality to a single
#: constant, so a Hebrew-localized fallback answer is exactly as
#: structurally enforced as the English one always has been.
APPROVED_INSUFFICIENT_EVIDENCE_FALLBACKS = frozenset(
    {EXACT_INSUFFICIENT_EVIDENCE_FALLBACK, EXACT_INSUFFICIENT_EVIDENCE_FALLBACK_HE}
)
...
class TutorAnswer(DomainModel):
    ...
    def _validate_answer(self) -> TutorAnswer:
        ...
        if (
            self.status == TutorAnswerStatus.FALLBACK
            and self.answer_markdown not in APPROVED_INSUFFICIENT_EVIDENCE_FALLBACKS
        ):
            raise ValueError("a FALLBACK answer must use one of the exact approved fallback texts")
        return self
