...
class AttemptRepositoryPort(Protocol):
    ...
    async def get_attempt_for_update(self, attempt_id: UUID) -> ExerciseAttempt | None:
        """Same as `get_attempt`, but acquires a row lock (`SELECT ... FOR
        UPDATE`) so a concurrent submitter blocks until this transaction
        commits or rolls back, instead of both readers seeing `STARTED`
        and racing to grade the same attempt."""
        ...
