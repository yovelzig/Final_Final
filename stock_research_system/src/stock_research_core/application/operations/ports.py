...
class JobExecutionContext:
    """Trusted job identity/requester context (Phase G2B).

    Constructed by `BackgroundJobService` from data it already trusts -
    `requested_by_account_id`/`requested_by_integration_id`/
    `idempotency_key` are `create_job`'s own authenticated keyword
    arguments, never a handler's own `parameters` (which may originate
    from an untrusted caller-supplied request body). Passed to both a
    job type's `resource_key_builder` (at creation time, before the job
    row - and for Live Research, before any domain `ResearchRequest` -
    exists) and to `JobHandlerPort.handle` (at execution time).

    `job_id` is generated by `create_job` *before* `resource_key_builder`
    runs, so the same id always matches the persisted `BackgroundJob` row.
    `attempt_number` is `0` in a creation-time context (no attempt has
    run yet) and the current attempt number (`job.attempt_count + 1`) in
    an execution-time context. `correlation_id` is sourced from the job's
    own `CREATED` event at execution time, since `BackgroundJob` itself
...
class JobHandlerPort(Protocol):
    ...
    async def handle(
        self, *, context: JobExecutionContext, parameters: Any, progress: ProgressReporterPort
    ) -> HandlerOutcome: ...
